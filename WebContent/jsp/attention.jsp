			...
			<header id="second">
					<nav  id="second_menu">
						...
						<!-- 导航栏部分_可选择部分 -->
							...
							<!-- 搜索框部分 -->
							<div class="search">
									<form action="FriendListServlet?method=search_attention&currentPage=1" id="search_action" method="post">
										<!-- placeholder占位符 -->
						 	...
						 	<c:forEach  items="${requestScope.p.objects}" var="u">
						 		...
						 		<div class="users_info_list">
						 			...
						 			<div id="everyone_list_operation">
						 				<a href="FriendListServlet?pre_method=unfollow&method=attention_list&toUserId=${u.userId}&currentPage=${requestScope.p.currentPage}" class="operation">取消关注</a>
						 				<a href="FriendListServlet?pre_method=black_on_record&method=attention_list&toUserId=${u.userId}&currentPage=${requestScope.p.currentPage}" class="operation">拉黑</a>
						 			</div>
						 		</div>
						 	</c:forEach>
						</div>
